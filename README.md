...
<p align="center">
  <img src="picture/p3.png" width="500"/>
</p>

在“常规-平台工具集”中选择LLVM(clang-cl)

<p align="center">
  <img src="picture/p4.png" width="500"/>
</p>
...
<p align="center">
  <img src="picture/p5.png" width="500"/>
</p>
...
<p align="center">
  <img src="picture/p6.png" width="500"/>
</p>

图1，实验比较结果

<p align="center">
  <img src="picture/p7.png" width="500"/>
</p>

 图二，前五个方法的CPU和内存分析

<p align="center">
  <img src="picture/p8.png" width="500"/>
</p>

图三：考虑最后一个OMP_Tiling_Collapse_Outer的CPU内存分析

<p align="center">
  <img src="picture/p9.png" width="500"/>
</p>

图四：截取内存分析快照

<p align="center">
  <img src="picture/p10.png" width="500"/>
</p>
...
### 4.2不同矩阵大小下的OpenMP方法性能对比

<p align="center">
  <img src="picture/p11.png" width="500"/>
</p>

矩阵乘法的MxNxK值：1024 x 1024x1024

<p align="center">
  <img src="picture/p12.png" width="500"/>
</p>

矩阵乘法的MxNxK值：512 x 512 x 512

<p align="center">
  <img src="picture/p13.png" width="500"/>
</p>
...
实验数据深层分析：
核心结论概述：随着问题规模扩大,算法对缓存层次结构的掌控能力，是决定GEMM性能扩展性的关键。相比单纯增加并行线程，围绕“数据局部性”的算法级优化，才能支撑大规模问题的持续性能收益。

各方案实验表现详情
1. 内存阶梯与访存墙的量化体现
观察Serial_Naive,当矩阵规模从512×512增至1024×1024时，理论计算量增长8倍，但运行时间却增长约17.10倍，呈现“超线性增长”特征。核心原因在于，矩阵规模超出L3缓存容量后，缓存失效问题严重，内存访问延迟取代计算速度成为主要性能瓶颈。

2. 并行边际效应
比较OMP_Naive和change_loop_order在1024规模的实验中，单线程循环换序（change_loop_order）方案获得了51.48倍的加速效果，而多线程并行（OMP_Naive）方案仅实现16.72倍的性能提升。由此可得出关键结论：访存优化属于“乘法级”优化，并行化属于“加法级”优化——若底层存在严重的缓存失效问题，单纯增加线程数量无法弥补内存访问的额外开销。

3. OMP_Tiling_Collapse_Outer_K实现
该方案表现出最优的性能扩展性：当矩阵规模翻倍时，运行时间仅增长5.97倍，远低于理论计算量对应的8倍增长。核心优势源于Tiling技术的应用：将大矩阵拆分为16×16的微块，确保数据能够驻留于缓存中，大幅减少缓存失效;同时，合理的微块划分也为编译器提供了指令级优化的有利条件，更不必说K先序带来的时空复杂度提升。从加速比表现来看，随着问题规模扩大，该方案的整体加速比从61倍提升至445倍，扩展性显著优于其他实现方案。

GEMM性能的核心制约因素的是数据局部性与缓存利用效率。在大规模矩阵乘法场景中，围绕“提升缓存命中率”的算法级优化（如Tiling、循环重排）,其性能增益远高于单纯的并行化优化。只有精准掌控缓存层次结构，优化内存访问模式，才能实现GEMM性能的持续提升


### 4.3不同线程数下的OpenMP方法性能对比

VCustomParam参数的传入方法：

注意到GEMM_cpu的调度函数是void GEMM_cpu(GEMM_common::GEMMCoreFunc vGEMMFunc, const std::string& vMethodName, const std::any& vCustomParam)
这里的最后一个参数就是线程数目的vCustomParam，我们只需要向这里传入即可
在main.cpp的main函数中，我们将创建有一个线程数目值的vCustomParam，并将这个any数据传入各个方法即可，代码如下：

```
int threadCount = 8;
    std::any customParam = threadCount;
    GEMM_cpu(GEMM_serial::change_loop_order, "Serial_Change_Loop_Order", customParam);
    GEMM_cpu(GEMM_openmp::naive, "OMP_Naive", customParam);
    GEMM_cpu(GEMM_openmp::collapse_manual, "OMP_Collapse_Manual", customParam);
    GEMM_cpu(GEMM_openmp::collapse, "OMP_Collapse", customParam);
GEMM_cpu(GEMM_openmp::collapse_tiled_outer_k, "OMP_Tiling_Collapse_Outer_K", customParam);
同时在代码里需要使用如下命令接收并将之应用在程序中
int threadCount = 1;
    if(vCustomParam.has_value()) {
        threadCount = std::any_cast<int>(vCustomParam);
    }
...
我们这里采用的矩阵大小为512 x 512 x 512, 分别讨论vCustomParam=1,8,16,32这4种情况

<p align="center">
  <img src="picture/p14.png" width="500"/>
</p>

 vCustomParam=1
 
 <p align="center">
  <img src="picture/p15.png" width="500"/>
</p>

vCustomParam=8

 <p align="center">
  <img src="picture/p16.png" width="500"/>
</p>

vCustomParam=16

 <p align="center">
  <img src="picture/p17.png" width="500"/>
</p>
...
| 实现方法 | 性能指标 | 1 线程 | 8 线程 | 16 线程 | 32 线程 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Serial_Naive**<br>(基准串行) | 耗时 (ms)<br>加速比 | 462.8<br>1.00x | 462.8<br>1.00x | 462.8<br>1.00x | 462.8<br>1.00x |
| **Serial_Change_Loop_Order**<br>(串行换序) | 耗时 (ms)<br>加速比 | 21.21<br>21.78x | 19.69<br>22.93x | 19.70<br>23.33x | 19.87<br>22.95x |
| **OMP_Naive**<br>(朴素并行) | 耗时 (ms)<br>加速比 | 574.0<br>0.81x | 70.0<br>6.61x | 38.0<br>12.18x | 33.0<br>14.02x |
| **OMP_Collapse**<br>(自动折叠) | 耗时 (ms)<br>加速比 | 568.0<br>0.82x | 71.5<br>6.47x | 36.0<br>12.86x | 31.0<br>14.93x |
| **OMP_Collapse_Manual**<br>(手动折叠) | 耗时 (ms)<br>加速比 | 562.0<br>0.82x | 69.8<br>6.63x | 35.5<br>13.04x | 30.5<br>15.17x |
| **OMP_Tiling_Collapse_Outer_K**<br>(分块并行) | 耗时 (ms)<br>加速比 | 18.94<br>24.38x | 5.094<br>88.64x | 2.476<br>186.25x | 2.595<br>175.73x |

各个方法间的比较在第一部分的性能分析已经完成，这里主要分析线程相关

实验分析

1.OMP_Naive和OMP_Collapse,OMP_Collapse_Manual的加速比都低于1，这证明了在没有充分利用并行的多线程处理优势时，并行库的管理线程和任务分配的开销会变得可观察。只有充分发掘并行能力才能弥补这一块的缺失

2.在从1线程到16线程的过程中，OMP_Naive和OMP_Collapse,OMP_Collapse_Manual以及OMP_Tiling_Collapse_Outer_K这一组的计算效率增长倍数与线程数量的增长倍数基本相对应（线程扩大多少倍，效率基本就提升多少倍），并且在约16线程处达到了巅峰状态。充分证明在这个区间内线程并行数量的不足是几乎最重要的性能限制瓶颈

3.从16倍提升到32倍时，OMP组的性能提升十分有限，特别是OMP_Tiling_Collapse_Outer_K方案甚至出现了轻微的效率下降，可以断定拐点发生在16.从16线程增长到32的瓶颈在于触碰到了硬件的物理资源瓶颈。由于大部分测试机的逻辑核心数目为16，增长到32将涉及到超线程技术，其让两个逻辑核共享了一个物理1执行单元和L1/L2缓存；在GEMM计算中，超线程带来的收益相当大程度在被两个逻辑核共享物理核带来的额外管理成本抵消（例如单元竞争和缓存抖动等问题）

4.从OMP_Tiling_Collapse_Outer_K来看，其在算法和缓存利用方面都做到了接近极致的优秀，因此在16->32线程管理时带来的多线程共享物理核的开销相对非常显著，造成了性能收益被开销完全盖过甚至出现了倒退。而对OMP_Collapse这样并不特别高效的方法，管理开销相对更下，16->32的提升收益并没有被完全该过

## 5 总结

计算规模：
由矩阵乘法的三重循环可知，一般计算复杂度是矩阵边长的三次方。在大型矩阵计算中，如果我们不适用效率优化，其会变得极其难以承受。同时根据实验的数据，如果我们的优化方法带来的开销显著没有被收益很好地覆盖，这个三次复杂度的常数因子还会更大（从假设的提升八倍复杂度变成了增加10倍复杂度）

并行线程：
理论上，一般机器可以有16个CPU物理核心供并行同时使用。如果超过这个数值，多逻辑核使用单物理核带来的开销会变得相当显著。我们一般就将并行线程数目设置为16即可最大程度利用CPU物理核心

GEMM_naive:
直接按照定义计算矩阵，不使用任何技术，作为正确性和效率检验。其耗时过大不可接受，我么采用逐步增量的实验研究了下面的各个提升技术，最终进行融合得出了最优解决方案

Serial_Change_Order:
A x B这一矩阵乘法没有良好地与硬件特性嵌合，A的时间局部性（因为是在内部循环每次访问不同的A[i][k])和B的空间局部性（B[k][j]相邻数值涉及不同行，违反了行优先存储）没有被充分利用，导致效率偏低。我们使用K_M_N的换序（A在最内循环每次访问同一个位置，而B访问的是同一列的内容）解决了这个问题，带来10倍以上的效率提升。

OMP_naive:
为了充分利用多核CPU的并行能力，且由于GEMM的不同部分结果计算是相互独立的，我们可以将不同任务交给多个CPU逻辑核处理，充分发挥其并行能力。对于大规模的矩阵，并行开销可以忽略不计。可以带来大约16倍的效能提升

OMP_collapse_manual:
手动实现，我们通过将外两层循环合并，内部将之使用触发核取模复原，使得可以被调度的循环数量更多。具体的循环合并介绍查看下方。

OMP_collapse
在使用了多线程并行的基础上，为了使得线程的计算任务分配更加合理均衡，我们将外两层核并成一层，使得算法能够有更多迭代调度，效能分配更加充分，从而提升并行核的使用率。这种优化是锦上添花，可以取得相对与并行比较微笑的效率提升（由合并后索引重计算等新的开销抵消了一部分）
OMP_collapse_tilled_outer_K:
其通过Tiling分块使得单个线程负责一个物理块，从而能够充分地调动缓存中的寄存器，将缓存的性能充分压榨（这一点在预实验中的内存使用率中十分明显），减小了计算时遇到的主存访问时间。同时，上述提到的并行，多线程（16），循环合并（外部块循环两层循环合并为一个），以及K_M_N的计算顺序等等方法也被综合使用在其中。在各个规模的GEMM计算中都表现出了最好的效果。
个人体会：
本次期末设计使得我对大规模计算有了更加深入的体会。不仅提高了代码的阅读核组织分析能力，同时使得我具备了从不同角度实验检测来分析方法的工程思维，对于OPenMP的使用也更加熟练。通过实践进一步对CPU的多核性利用，循环合并的好处，缓存的充分使用（Tiling)以及算法性优化提升时空局部利用性这几个方面提升了认知。为我之后进一步阶段可能的并行计算深入学习打下了良好的基础

...
Serial_Naive	耗时 (ms)	25.52	465.09	7954.30	性能下限：倍数关系高达 17-18x，远超计算量增长的 8x 理论值。这量化证明了由于 Cache Miss 严重，CPU 绝大部分时间在等待数据而非计算。
(基准测试)	倍数关系	1	18.23	17.10	
	加速比	1.00x	1.00x	1.00x	
---	---	---	---	---	:---
Change_Loop_Order	耗时 (ms)	2.37	20.21	154.50	访存质变：仅通过改变循环顺序，使倍数关系回归至理论值（8x）。结论：好的串行逻辑比盲目的并行更有效，它在 1024 规模下胜过了所有朴素并行方案。
(换序优化)	倍数关系	1	8.52	7.65	
	加速比	10.76x	23.02x	51.48x	
---	---	---	---	---	:---
OMP_Naive	耗时 (ms)	2.92	39.58	475.88	带宽瓶颈：加速比随规模增大而提升缓慢（8x 到 16x）。结论：在未解决访存冲突前，增加核心数会加剧总线竞争，导致多核效率随规模增大而边际递减。
(朴素并行)	倍数关系	1	13.55	12.02	
	加速比	8.73x	11.75x	16.72x	
---	---	---	---	---	:---
OMP_Collapse	耗时 (ms)	2.73	40.05	441.41	调度优化：加速比略优于 Naive。结论：通过合并循环维度提升了任务颗粒度，改善了多核负载均衡，但由于没解决底层访存模式，无法产生质的突破。
(系统自带折叠)	倍数关系	1	14.67	11.02	
	加速比	9.35x	11.61x	18.02x	
---	---	---	---	---	:---
Collapse_Manual	耗时 (ms)	2.74	40.12	428.47	调度天花板：性能与系统版基本一致。结论：手动划分任务虽精准，但在访存受限型任务中，调度层面的优化已达极限，无法抵消 Cache 失效带来的损耗。
(手动计算折叠)	倍数关系	1	14.64	10.68	
	加速比	9.31x	11.59x	18.56x	
---	---	---	---	---	:---
OMP_Tiling...	耗时 (ms)	0.41	2.99	17.86	全场最优：倍数关系（5.97x）甚至跑赢了理论值（8x）。结论：通过 Tiling 实现了缓存复用，彻底解除访存限制，使 CPU 算力得以满额释放，是唯一具备大规模扩展性的方案。
(分块综合优化)	倍数关系	1	7.29	5.97	
	加速比	61.91x	155.74x	445.41x	
